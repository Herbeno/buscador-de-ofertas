...
def analyze_data(file_path="data/raw_data.csv"):
    ...
    # 1. Filtro de Preço (Remover acessórios e anúncios absurdos)
    # Uma RTX 3060 nova/usada dificilmente custa menos de R$ 1400 ou mais de R$ 4500
    df_clean = df[(df['preco'] >= 1400) & (df['preco'] <= 4500)].copy()
    
    # 2. Filtro de Palavras-Chave (Remover "caixas", "defeitos", etc)
    ...
    if not ofertas.empty:
        ...
        print("Nenhuma oferta excepcional encontrada no momento.")
